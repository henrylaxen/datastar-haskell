...
send eventType dataLines options = catMaybes [e,d,i,r]
  let
    e = withJust (toBuilder eventType)
    d = wiothJust (concatMap withData dataLines)
    i = if eventId options == mempty then Nothing else withJust (withColon cEventId) <> eventId options
    r = if retryDuration options == cDefaultSseRetryDurationMs then Nothing else withJust (withColon cRetryDuration) <> retryDuration options
